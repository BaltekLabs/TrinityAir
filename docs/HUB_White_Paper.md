# T&P NEXUS HUB TRANSPORTATION SYSTEM
## White Paper: America's First Integrated eVTOL–Autonomous Vehicle Mobility Hub

**Fort Worth, Texas | March 2026**

---

> **Policy Context — March 9, 2026:** The U.S. Department of Transportation and FAA announced 8 selections for the White House eVTOL Integration Pilot Program (eIPP). Texas DOT, with Archer Aviation as its industry partner, is one of the 8. Named routes connect Dallas, Austin, San Antonio, and Houston. Archer Midnight air taxi operations begin in Texas **H2 2026**. Fort Worth — Texas's second million-plus city and the DFW corridor's western anchor — is not yet on the route map. The T&P Nexus HUB is the infrastructure project that puts it there.

---

## TABLE OF CONTENTS

1. Executive Overview
2. The Problem: DFW's Transportation Crisis
3. The Inflection Point: eVTOL and AV Arrive Simultaneously
4. The Solution: T&P Nexus HUB
5. The Asset: Texas & Pacific Warehouse
6. Technical Architecture
7. Market Analysis
8. Global Precedents
9. Implementation Roadmap
10. Financial Model
11. Funding Strategy
12. Risk Assessment
13. Regulatory Pathway
14. Stakeholder Value Framework
15. Conclusion and Call to Action

---

## 1. EXECUTIVE OVERVIEW

The T&P Nexus HUB Transportation System proposes the adaptive reuse of the historic Texas & Pacific Warehouse in downtown Fort Worth, Texas, into the nation's first fully integrated multi-modal mobility hub — combining electric vertical takeoff and landing (eVTOL) aircraft, Level 4 autonomous electric vehicles, a 120,000 square foot technology innovation center, and AI-powered systems management into a single, seamlessly connected infrastructure platform.

This is not a speculative concept. The technologies that power the HUB are commercially operational today. Waymo delivers 450,000+ paid autonomous rides per week across five U.S. cities including Austin, Texas. Archer Aviation holds FAA Part 135 Air Carrier, Part 145 Repair Station, and Part 141 Pilot Training certificates and begins Texas air taxi operations under the federal eIPP program in H2 2026. Joby Aviation completed FAA Stage 4 of 5 type certification in November 2025 and targets first U.S. commercial passenger flights in 2026.

The HUB arrives precisely at the moment these technologies transition from demonstration to deployment. Fort Worth — ranked the 10th worst commute city in America, home to 1,008,106 residents, and growing by approximately 487 people per day — is the right city. The T&P Warehouse, an 8-story, 600-foot-long 1920s railroad landmark sitting vacant in the heart of downtown, is the right building. And the window, triggered by the federal eIPP announcement, is now.

**Investment Summary**

| Item | Figure |
|------|--------|
| Total Capital Required | $129M across 4 phases (2026–2029) |
| Projected IRR (moderate) | 22.3% |
| Revenue at Year 5 (moderate) | $133.4M |
| Break-Even | Year 5 (2030) |
| NPV at 10% discount rate | $89M |
| Payback Period | 4.2 years |
| Jobs Created | 2,500+ construction; 800+ permanent |
| Projected Annual Tax Revenue | $25M+ by Year 5 |

---

## 2. THE PROBLEM: DFW'S TRANSPORTATION CRISIS

The Dallas-Fort Worth metroplex is simultaneously one of America's greatest economic success stories and one of its most acute transportation failures. It is the third-largest metro economy in the United States, generating more than $710 billion in annual GDP with 5.7% annual growth. It is also the 4th largest metro by population at 8.5 million residents, growing faster than any comparable U.S. city.

That growth is strangling itself.

### The Numbers

- **174 million hours** of collective traffic delay in DFW in 2024 — up 8% year-over-year
- **69 hours lost per commuter annually** — 10% above the national average, costing each driver $1,618 in wasted fuel and productivity
- **44 of the 100 most congested roads in Texas** are in the DFW metroplex
- **Less than 1% of workers** use public transit; approximately 70% drive alone
- **$36.4 billion** projected annual congestion cost to the DFW economy by 2050 if no transformative action is taken
- **$431.5 billion** in infrastructure investment needed by 2040 — only 30% currently funded
- Fort Worth ranked **10th worst commute city** in the United States (Forbes, 2025)

### Why Conventional Solutions Fail

Highway expansion has been the default response for decades. It has not worked. The Texas Transportation Institute documents a consistent pattern: new lane capacity induces new demand within five to seven years, restoring congestion to prior levels. Rail expansion is politically constrained, capital-intensive, and optimized for fixed corridors rather than the distributed, point-to-point demand patterns of a sprawling metroplex like DFW.

The problem is not just volume. It is geometry. DFW is an 8,000-square-mile metropolitan area with no geographic constraints — no rivers, no mountains, no coastline to focus development. Destinations are scattered across hundreds of municipalities. No surface-based transit network can efficiently serve this topology at scale.

The solution requires a new dimension: altitude.

---

## 3. THE INFLECTION POINT: eVTOL AND AV ARRIVE SIMULTANEOUSLY

Two technology categories that were theoretical five years ago are commercially operational today. Their simultaneous maturity creates a narrow window for the HUB's first-mover advantage.

### Electric Vertical Takeoff and Landing (eVTOL) Aircraft

The eVTOL industry has moved decisively from demonstration to deployment.

**Archer Aviation** holds three FAA certificates — Part 135 Air Carrier (revenue passenger operations), Part 145 Repair Station (maintenance authority), and Part 141 Pilot Training (crew certification). Its Midnight aircraft carries 4 passengers plus pilot at 150 mph cruise speed with a 60-mile range. Archer is the Texas partner in the federal eIPP and begins revenue operations in Texas in H2 2026. At scale, Archer targets 10–25 trips per aircraft per day, with a fare model of approximately $3–11 per mile — turning a 60-to-90-minute DFW car trip into a 10-to-20-minute flight.

**Joby Aviation** completed FAA Stage 4 of 5 Type Inspection Authorization in November 2025 — the furthest any eVTOL manufacturer has reached in the U.S. certification process. Its S4 aircraft carries 4 passengers plus pilot at up to 200 mph with a 100-mile range and 65 dB noise floor (comparable to a normal conversation). Joby is also a Texas eIPP participant and targets first U.S. commercial passenger operations in 2026. Toyota has invested more than $1 billion in Joby.

**The eIPP Program** is the critical regulatory unlock. Established by Presidential Executive Order in September 2025, it authorizes participating operators to carry revenue passengers *before* full FAA type certification is complete — something previously unavailable under research and development programs. This is real commercial service, beginning this year.

**Industry note:** Lilium declared insolvency in November 2024 after burning $1.8 billion. This outcome validates the HUB's platform-agnostic design philosophy — the infrastructure serves multiple operators, insulating the project from single-manufacturer risk.

### Autonomous Ground Vehicles

Level 4 autonomous vehicles — operating without human supervision in defined service areas — are no longer a prototype technology. They are a commercial product.

**Waymo** operates 450,000+ paid rides per week commercially in Phoenix, San Francisco, Los Angeles, Austin (Texas), and Atlanta. It raised $16 billion at a $126 billion valuation in February 2026 and is expanding to six additional cities. Its safety record, documented across 25.3 million autonomous miles, shows 88% fewer property damage claims and 92% fewer injury claims than human-driven vehicles (Swiss Re study).

Waymo's **Austin, Texas** deployment — just 30 miles from the Fort Worth core — establishes a direct regional precedent and the clearest possible expansion pathway into DFW.

### The Integration Gap

Both technologies are arriving simultaneously in the DFW market with no integrated infrastructure to connect them. Air mobility without ground connectivity creates passenger experience friction that limits adoption. Ground mobility without air speed fails to solve DFW's geometry problem. The HUB closes that gap — providing the single passenger experience that makes both technologies more useful together than either is alone.

No comparable integrated eVTOL-AV hub currently operates in any major U.S. metropolitan area. The HUB would be the first.

---

## 4. THE SOLUTION: T&P NEXUS HUB

The T&P Nexus HUB integrates four functional systems within a single historic building and its immediate environs:

### 4.1 eVTOL Air Mobility Terminal
Rooftop vertiport infrastructure supporting multiple simultaneous aircraft — landing pads, charging systems, passenger boarding facilities, operations and air traffic coordination center, and weather monitoring systems. Initial fleet of 10–15 aircraft (Archer Midnight and/or Joby S4) scaling to 25–40 aircraft by Year 5. The rooftop provides line-of-sight visibility across downtown Fort Worth and direct connection to the T&P passenger terminal below.

### 4.2 Autonomous Vehicle Ground Terminal
Ground-level EV fleet operations providing first- and last-mile connectivity across the Fort Worth urban core, connecting the HUB to residential neighborhoods, commercial districts, the Texas A&M Fort Worth campus corridor, and the TEXRail commuter rail network. Fleet of 50–100 Level 4 autonomous vehicles at full operations. Smart charging infrastructure integrated with on-site power systems.

### 4.3 Technology Innovation Center
Floors 2 through 7 — approximately 120,000 square feet of leasable research and development space — providing a revenue-generating innovation ecosystem for transportation technology companies, academic research partners, battery technology developers, robotics labs, and AI research programs. Fort Worth Class A downtown rental rates of $29–$37 per square foot. The HUB serves simultaneously as an operational transportation system and a living laboratory — the world's first vertiport-scale research deployment platform.

### 4.4 AI-Powered Systems Integration
A central command platform manages real-time scheduling across air and ground fleets, predictive maintenance, weather-adaptive routing, demand forecasting, and seamless passenger handoff between VTOL and AV services. A single consumer-facing application books, tracks, and pays for end-to-end journeys. This integration layer is the HUB's most durable competitive asset — proprietary data and operational expertise that deepens over time.

---

## 5. THE ASSET: TEXAS & PACIFIC WAREHOUSE

The T&P Warehouse is not a liability. It is the project's most important strategic asset.

Built in the 1920s to serve the Texas & Pacific Railway at the height of Fort Worth's railroad era, the building is an 8-story, 600-foot by 100-foot masonry and concrete structure — approximately 480,000 total square feet — located in the heart of downtown Fort Worth. Its rooftop is large enough for multiple simultaneous VTOL operations. Its floor plates are designed for heavy industrial loads. Its basement and upper floors provide space for computing infrastructure and research facilities. Its location at the convergence of downtown Fort Worth, the Panther Island development, and TEXRail commuter rail is unmatched.

The building has been vacant since 1998 — a 27-year history of announced redevelopment plans that never materialized, including a terminated $11M+ Tax Increment Finance agreement in 2016. The City of Fort Worth has characterized its condition as "demolition by neglect." A structural assessment conducted in 2023 identified $2.15 million in stabilization work as an immediate requirement.

That assessment is the project's foundation, not its barrier. The structural core of the building is sound. The required stabilization work is defined and budgeted. And the building's vacancy means the HUB acquires it without the displacement or demolition costs that would attach to an occupied site.

The T&P Warehouse is a historic transportation building that spent a century moving people by rail. The HUB's proposal is, in the deepest sense, a continuation of its original purpose — updated for the next century of mobility.

---

## 6. TECHNICAL ARCHITECTURE

### Aircraft Operations
- **Landing infrastructure:** Reinforced rooftop pads rated for eVTOL gross weights (Archer Midnight: ~3,175 lbs MTOW); drainage, lighting, and wind monitoring systems
- **Charging systems:** High-speed DC charging (350–500 kW per pad) enabling rapid turnaround between flights
- **Air traffic management:** Integration with FAA's UTM (Unmanned Traffic Management) framework and low-altitude airspace coordination systems
- **Passenger flow:** Dedicated elevator core connecting rooftop operations to ground-level terminal; security screening integrated into boarding process

### Ground Vehicle Operations
- **AV terminal:** Covered, dedicated curbside with real-time dispatch and passenger matching
- **Charging depot:** Overnight and rapid-charge infrastructure for full fleet
- **Fleet management:** Real-time vehicle tracking, predictive dispatch, and maintenance scheduling
- **TEXRail integration:** Direct pedestrian connection enabling air-rail-AV intermodal journeys

### Building Systems
- **Structural rehabilitation:** $2.15M stabilization work (standing water remediation, code compliance, structural reinforcement)
- **Power infrastructure:** Dedicated electrical service upgrade to support simultaneous aircraft charging and computing loads; solar canopy potential on rooftop
- **Computing infrastructure:** Top-floor and basement data center space for AI traffic management and partner hosting
- **Safety systems:** Emergency response protocols, fire suppression, crash/rescue equipment, and 24/7 operations monitoring

### Data and Software Platform
- **Unified booking app:** Single interface for VTOL + AV journey planning, ticketing, and real-time tracking
- **AI optimization engine:** Demand prediction, dynamic pricing, route optimization, and maintenance scheduling
- **API layer:** Partner integrations with airline booking systems, ride-share platforms, and corporate travel management tools
- **Data flywheel:** Operational data from millions of passenger-miles continuously improves scheduling accuracy and safety performance

---

## 7. MARKET ANALYSIS

### 7.1 Total Addressable Market

The urban air mobility market was valued at $4.6 billion in 2024 and is projected to reach $23.5–$29.2 billion by 2030 and $41.5 billion by 2035, at a compound annual growth rate of 31–34% (MarketsandMarkets, Grand View Research, 2025). Morgan Stanley projects the broader urban mobility ecosystem — incorporating AV, UAM, and related infrastructure — to exceed $1 trillion by 2040. North America commands the largest regional share globally at 40%+.

### 7.2 DFW Serviceable Market

The DFW metropolitan area provides an unusually concentrated serviceable market:

- **8.5 million residents** — 4th largest U.S. metroplex, growing at ~487 people per day
- **$710+ billion in regional GDP** — 3rd largest metro economy in the United States
- **600,000 jobs created** in DFW in the five years prior to 2024 (Dallas Federal Reserve)
- **Major corporate concentration:** American Airlines, AT&T, Southwest Airlines, Charles Schwab, Toyota North America, and dozens of Fortune 500 regional headquarters within the service radius

At even 0.5% modal shift in DFW commute trips — a conservative target given Waymo's demonstrated 450,000 weekly rides in smaller markets — the HUB serves hundreds of thousands of passengers annually.

### 7.3 Target Customer Segments

**Business travelers and commuters** represent the primary revenue driver in Phase 1 and 2. The HUB's $75–$150 per trip pricing is competitive with current business travel expenses (Uber Black, parking, fuel) and directly targets the high-value-per-mile customer segment that eVTOL economics require to reach profitability.

**Airport connectivity** between DFW International Airport, Love Field, and downtown Fort Worth represents a high-frequency, price-inelastic demand segment. Current ground transportation from downtown Fort Worth to DFW Airport requires 30–50 minutes by car. An eVTOL connection would complete the journey in under 12 minutes.

**Innovation center tenants** provide stable, recurring real estate revenue independent of transportation adoption curves. The DFW office market commands $29–$37 per square foot for Class A downtown space. A purpose-built transportation innovation cluster — co-located with an operational vertiport and AV terminal — commands a premium.

**Cargo and delivery** represent an expansion revenue stream beginning in Phase 4, leveraging established aircraft operations infrastructure for last-mile urban logistics.

---

## 8. GLOBAL PRECEDENTS

The HUB does not ask the question "can this work?" — it asks "what does doing this well look like?" Global case studies provide the answers.

### Singapore: Regulatory-First AV Integration
Singapore established dedicated autonomous vehicle test zones with structured government support, phased regulatory development, and mandatory stakeholder collaboration from inception. The result was the fastest commercial AV deployment timeline of any major city. Lesson: regulatory relationships built early, not acquired reactively, determine deployment speed.

### Dubai: Urban Air Mobility Infrastructure
Dubai's Roads and Transport Authority committed to vertiport infrastructure investment before commercial eVTOL certification was complete — securing first-mover advantage in airspace management and passenger experience design. The lesson for Fort Worth: infrastructure investment that precedes operator arrival sets the terms of the partnership.

### Seoul: AI Traffic Management at Scale
Seoul's integrated digital twin and AI traffic management platform — connecting thousands of sensors, vehicles, and data streams in real time — demonstrated a 20%+ reduction in urban congestion on managed corridors. The HUB's AI integration layer is modeled on this approach, adapted for a U.S. regulatory and urban context.

### London: Historic Station as Mobility Hub
King's Cross and Paddington stations in London demonstrate that adaptive reuse of historic transportation buildings as modern multi-modal hubs is not only feasible but creates iconic civic anchors. Both conversions preserved and celebrated historic character while adding contemporary transit functions. The T&P Warehouse has equivalent architectural significance and structural potential.

### Los Angeles Union Station: AV Integration
LA Metro's Union Station mobility hub — integrating rail, bus, AV testing, and adjacent development — is the closest U.S. analog to the HUB concept. Its public-private partnership structure, phased approach, and multi-modal integration architecture provide a direct template for the Fort Worth implementation.

---

## 9. IMPLEMENTATION ROADMAP

### Phase 1: Foundation (2026) — Active
**Investment: $15–25M | Duration: 12 months**

The foundation phase establishes the regulatory, partnership, and physical prerequisites for construction. Key activities include:

- FAA eIPP enrollment and vertiport design coordination
- T&P Warehouse acquisition or long-term ground lease
- $2.15M structural stabilization (standing water remediation, code compliance)
- Memoranda of Understanding with eVTOL operators (Archer, Joby) and AV partner (Waymo or equivalent)
- City of Fort Worth municipal partnership and public-private agreement framework
- Texas DOT eIPP program coordination — positioning T&P Warehouse as the Fort Worth network anchor
- Phase 1 equity and grant funding close ($20M target)
- Architectural and engineering design completion
- Team assembly: CEO, COO, CTO, VP of Operations, Government Relations

**Phase 1 Milestone:** Property secured, funding closed, regulatory pathway established, technology partner MOUs signed.

### Phase 2: Infrastructure Development (2027)
**Investment: $40–60M | Duration: 18 months**

The infrastructure phase transforms the T&P Warehouse from a vacant building into an operational mobility facility:

- Historic building rehabilitation — floors 1–7, building envelope, MEP systems
- Rooftop vertiport construction: landing pads, charging systems, operations center, weather monitoring
- Ground-level AV terminal: covered curbside, dedicated charging depot, passenger matching systems
- Computing and telecommunications infrastructure (floors 8 and basement)
- Safety, fire suppression, and emergency response systems installation
- Innovation center tenant improvement — Phase 1 tenants: 40,000–60,000 sq ft
- AI traffic management platform deployment (initial version)
- Community engagement and public education programming

**Phase 2 Milestone:** Building fully rehabilitated and operational, vertiport infrastructure certified, first innovation center tenants occupying.

### Phase 3: Technology Integration and Pilot Operations (2028)
**Investment: $25–40M | Duration: 12 months**

Technology deployment and operational validation:

- eVTOL fleet acquisition: 10–15 aircraft at approximately $1.3M per unit (Archer Midnight and/or Joby S4)
- Level 4 AV fleet deployment: 25–50 vehicles
- Full AI systems integration and real-time operations launch
- FAA operational approval for commercial revenue eVTOL operations
- Consumer-facing app launch and marketing campaign
- Pilot operations: limited routes, selected customer segments, rigorous safety data collection
- Innovation center at 70%+ occupancy

**Phase 3 Milestone:** Pilot operations active, safety certification achieved, 1,000+ pilot program participants, break-even operational model validated.

### Phase 4: Full Operations and Expansion (2029+)
**Investment: $20–25M | Duration: 12 months**

Full commercial launch and network growth:

- Full eVTOL commercial operations: 25–40 aircraft fleet
- Full AV network: 50–100 vehicles serving DFW urban core
- Airport connectivity routes: DFW International and Love Field
- Cargo and delivery service launch
- Secondary hub development: DFW Airport, Alliance, Las Colinas
- Technology licensing revenue activation
- National replication planning for five additional metropolitan markets

**Phase 4 Milestone:** 10,000+ daily passengers, 5+ secondary hubs operational, technology licensing revenue active, national expansion planning complete.

---

## 10. FINANCIAL MODEL

### Revenue Projections — Year 5 (2030)

| Revenue Stream | Conservative | Moderate | Optimistic |
|----------------|-------------|----------|------------|
| eVTOL Operations | $21.9M | $54.8M | $131.4M |
| Autonomous Vehicle Services | $16.4M | $54.8M | $137.3M |
| Innovation Center / Real Estate | $2.4M | $3.8M | $5.1M |
| Technology Licensing & IP | $4.0M | $10.0M | $16.0M |
| Ancillary (retail, cargo, parking) | $6.0M | $10.0M | $16.0M |
| **Total Revenue** | **$50.7M** | **$133.4M** | **$305.8M** |

### Revenue Model Assumptions (Moderate Scenario)
- **eVTOL:** 30 aircraft × 10 flights/day × 5 passengers × $100 average fare × 365 days
- **AV Services:** 75 vehicles × 20 trips/day × 5 passengers × $20 average fare × 365 days
- **Innovation Center:** 120,000 sq ft × 90% occupancy × $35/sq ft annual rent
- **Technology Licensing:** Proprietary AI platform licensing to secondary hub operators

### Operating Cost Structure — Year 5 (Moderate Scenario)

| Cost Category | Annual Cost |
|---------------|-------------|
| Personnel (pilots, technicians, operations, technology, admin) | $32.5M |
| Aircraft maintenance | $8–15M |
| AV fleet maintenance and operations | $2–4M |
| Technology infrastructure | $3–6M |
| Energy (electricity, charging) | $8–12M |
| Insurance (aviation, general, cyber, property) | $4–8M |
| Facility operations | $2–3M |
| Regulatory compliance | $1–2M |
| Marketing and customer acquisition | $3–5M |
| Debt service | $5–8M |
| **Total Operating Expenses** | **~$88.5M** |

### Profitability (Moderate Scenario, Year 5)

| Metric | Value |
|--------|-------|
| Revenue | $133.4M |
| EBITDA | $44.9M |
| EBITDA Margin | 34% |
| Net Income | $16.4M |
| Projected IRR | 22.3% |
| NPV (10% discount) | $89M |
| Payback Period | 4.2 years |

### 5-Year Cash Flow Summary

| Year | Phase | Investment | Revenue | Net Cash Flow | Cumulative |
|------|-------|------------|---------|---------------|------------|
| 2026 | 1 | ($20M) | $0 | ($25M) | ($25M) |
| 2027 | 2 | ($50M) | $5M | ($60M) | ($80M) |
| 2028 | 3 | ($32M) | $25M | ($37M) | ($92M) |
| 2029 | 4 | ($22M) | $65M | ($2M) | ($29M) |
| 2030 | 5 | ($5M) | $133M | $40M | **+$11M** |

### Sensitivity Analysis — IRR vs. Key Variables

| Variable | −20% | −10% | Base | +10% | +20% |
|----------|------|------|------|------|------|
| Ridership | 15.8% | 19.1% | 22.3% | 25.4% | 28.6% |
| Average Fare | 18.7% | 20.5% | 22.3% | 24.1% | 25.9% |
| Operating Costs | 26.1% | 24.2% | 22.3% | 20.4% | 18.5% |
| Capital Costs | 24.8% | 23.5% | 22.3% | 21.1% | 19.9% |

The project maintains double-digit IRR across all meaningful sensitivity scenarios, demonstrating structural financial resilience.

---

## 11. FUNDING STRATEGY

### Capital Structure

| Source | Amount | % of Total |
|--------|--------|------------|
| Equity (angels, VC, PE, strategic) | $58M | 45% |
| Debt (commercial, equipment, municipal bonds) | $45M | 35% |
| Grants (federal, state, local) | $20M | 15% |
| Operating Revenue (later phases) | $6M | 5% |
| **Total** | **$129M** | **100%** |

### Grant Opportunities
The eIPP designation creates direct federal funding pathways that did not exist before March 9, 2026:

- **USDOT Advanced Transportation Technology and Innovation (ATTAIN):** $3M target
- **FAA Urban Air Mobility grants:** $2M target
- **DOE Vehicle Technology Program:** $1.5M target
- **Texas Economic Development Act:** $2.5M target
- **Texas Enterprise Fund:** $2M target
- **City of Fort Worth Economic Development Incentives:** $2M target
- **Historic Tax Credits (Federal, 20% of qualified expenses):** Potentially $10M+ given T&P Warehouse eligible renovation scope

### Strategic Investment Targets
- eVTOL manufacturers (Joby, Archer) as equity co-investors aligned with their Texas deployment
- Real estate infrastructure funds (Blackstone Infrastructure, Brookfield, Macquarie)
- Texas-based family offices and energy sector capital seeking transportation technology exposure
- Toyota and Hyundai, both of whom have demonstrated willingness to invest directly in eVTOL and AV ecosystems

---

## 12. RISK ASSESSMENT

### Risk Summary Matrix

| Risk | Impact | Probability | Score | Status |
|------|--------|-------------|-------|--------|
| FAA approval delays | Major | Medium (30%) | 12 | Managed |
| VTOL technology maturity | Major | Medium (35%) | 12 | Managed |
| Construction cost overruns | Moderate | High (60%) | 12 | Managed |
| Market adoption rate | Major | Medium (40%) | 12 | Managed |
| Cybersecurity incident | Major | Low (15%) | 8 | Monitored |
| Competitor market entry | Major | Low (20%) | 8 | Monitored |
| AV integration challenges | Moderate | Low (20%) | 6 | Low |
| Safety incident | Critical | Very Low (<5%) | 5 | Managed |
| Environmental opposition | Moderate | Low (20%) | 6 | Low |

No critical (21–25) or high (13–20) risk items are identified. All medium risks are actively managed through the mitigations below.

### Primary Risk Mitigations

**Regulatory risk:** Early FAA engagement; eIPP participation from inception; hire experienced aviation regulatory counsel; buffer regulatory timelines by 25% in project schedule.

**Technology risk:** Platform-agnostic design supports multiple eVTOL operators; phased deployment begins AV operations before eVTOL is required; lease vs. purchase flexibility on aircraft.

**Construction risk:** 15–20% contingency reserve in Phase 2 budget; fixed-price contracts where possible; experienced historic renovation contractors; comprehensive construction insurance.

**Adoption risk:** Conservative ridership assumptions in base financial model; multiple revenue streams (real estate, technology licensing, ancillary) provide cushion while transportation adoption ramps; eIPP creates regulatory certainty that accelerates adoption.

### Insurance Requirements
Total insurance budget of $2–4M annually, covering: aviation liability ($100M per occurrence), general liability ($25M), product liability ($50M), cyber liability ($10M), property ($150M replacement cost), and business interruption ($25M).

---

## 13. REGULATORY PATHWAY

### Federal
**FAA eIPP Participation:** The HUB's first regulatory priority is formal inclusion in the Texas eIPP program — securing vertiport designation for the T&P Warehouse site within the Archer Aviation and Joby Aviation Texas operations network. This requires engagement with TxDOT's Emerging Aviation Technology Section (Director Sergio Roman; AAM Lead Emily Lambert) and direct coordination with the eIPP operator teams.

**Vertiport Design Standards:** FAA Advisory Circular 150/5390-3 establishes vertiport design guidance. The rooftop design will comply with all clearance, marking, lighting, and obstruction requirements, and will be submitted for FAA verification prior to construction.

**UTM Integration:** The HUB's air traffic management platform will integrate with FAA's Unmanned Traffic Management (UTM) framework, coordinating with TxDOT's UAS Program Manager (William Graffis) for low-altitude airspace coordination.

### State
**TxDOT Coordination:** TxDOT Aviation Division's Emerging Aviation Technology Section is the primary state regulatory partner. The HUB will seek TxDOT's support in vertiport designation and state-level eIPP program integration.

**TxDOT Ground Operations:** The Texas Department of Transportation also governs curb management and right-of-way access for the AV terminal. Early coordination with TxDOT's Urban Mobility team is required for the Lancaster Avenue curb designation.

### Local
**City of Fort Worth:** Zoning (current designation: MU-2 Mixed Use; VTOL operations will require specific use authorization or a new category); building permits; historic preservation compliance with the Historic and Cultural Landmarks Commission.

**Historic Preservation:** The T&P Warehouse's historic significance triggers Section 106 review for any federally funded work. This review, while substantive, is also the gateway to the 20% Federal Historic Tax Credit — a significant capital source.

---

## 14. STAKEHOLDER VALUE FRAMEWORK

### Government and Public Sector
The HUB delivers to Fort Worth and Texas the economic infrastructure of a national transportation innovation cluster:

- **$500M+ economic multiplier** from hub operations and surrounding development catalysis
- **2,500+ construction jobs** during build-out; 800+ permanent positions at full operations
- **$25M+ annual tax revenue** from property, payroll, and sales taxes by Year 5
- **Historic preservation** of the T&P Warehouse — resolving a 27-year vacancy problem
- **Transportation innovation leadership** positioning Fort Worth as a global peer of Dubai, Singapore, and London in next-generation mobility
- **Federal eIPP positioning** — the opportunity to become the Fort Worth anchor of a federally sanctioned, nationally significant aviation program

### Investors and Financial Partners
- **22.3% projected IRR** (moderate scenario) with multiple exit pathways: strategic acquisition, IPO, or infrastructure fund recapitalization
- **First-mover positioning** in the $23–41B+ urban air mobility market before the competitive landscape consolidates
- **Scalable and replicable model** — the HUB's technology platform and operational playbook is designed for deployment across 50+ comparable U.S. metropolitan markets
- **Diversified revenue streams** reducing dependency on any single service line during the adoption curve
- **Federal validation** via eIPP dramatically reduces regulatory risk premium

### Technology Partners
- **Real-world operational deployment** at scale — the most valuable validation asset in the eVTOL and AV industries
- **Reference customer status** in a landmark project for global sales and marketing leverage
- **Joint R&D opportunity** in a living laboratory operating at the frontier of transportation integration
- **Market access** to 8.5 million DFW residents and the $710B+ regional economy

### Community and the Public
- **40–60% commute time reduction** for HUB users; measurable contribution to Fort Worth's 10th-worst commute ranking
- **30% reduction in local traffic emissions** from modal shift to electric air and ground transportation
- **Historic preservation** of an iconic building that defines downtown Fort Worth's visual identity
- **800+ permanent jobs** with training and development programs emphasizing local hiring
- **Public innovation showcase** on the ground floor — STEM programs, demonstrations, and educational access to cutting-edge transportation technology

---

## 15. CONCLUSION AND CALL TO ACTION

The T&P Nexus HUB is not a project waiting for technology to catch up. The technology has arrived. Archer Aviation holds its air carrier certificate and begins Texas operations this summer. Waymo operates commercially in Austin today. The federal government has declared Texas a national priority for eVTOL integration and selected it as one of only eight sites nationwide.

What is missing is the infrastructure that ties it together in Fort Worth — a physical hub, an integrated operations platform, and the partnership coalition to build and operate it.

The T&P Warehouse has waited 27 years for a project worthy of it. It was built to move people, and it will move people again — at 150 miles per hour, silently, over the rooftops of a city that is growing faster than its roads can carry it.

The window opened on March 9, 2026. It will not stay open indefinitely.

### Immediate Opportunities

**For Government Partners:** We are seeking a City of Fort Worth municipal partnership agreement, a letter of support for eIPP vertiport designation, and coordination with TxDOT's Emerging Aviation Technology Section. The city's first action — a letter to TxDOT and U.S. DOT supporting Fort Worth's inclusion in the Texas eIPP network — costs nothing and positions the city decisively.

**For Investors:** Phase 1 financing of $20M is actively raising, targeting close by Q3 2026. The eIPP announcement has materially de-risked the regulatory and technology assumptions underlying the financial model. Investor materials including the complete financial model, risk assessment, and technology partnership term sheets are available upon request.

**For Technology Partners:** We are executing Memoranda of Understanding with eVTOL operators and autonomous vehicle providers in Q2 2026. If your organization is deploying or planning to deploy in Texas, the HUB is the Fort Worth anchor you need.

**For the Community:** The T&P Warehouse belongs to Fort Worth. This project is built to serve Fort Worth first — and to show the world what Fort Worth is capable of building.

---

*For partnership inquiries, investment discussions, or government coordination:*
*partnerships@tpnexus.com*

---

**Data Sources:** U.S. Census Bureau; Dallas Federal Reserve; Texas A&M Transportation Institute Urban Mobility Report 2025; MarketsandMarkets Urban Air Mobility Report 2025; Grand View Research; Morgan Stanley; CommercialCafe Fort Worth Market Data 2025; Forbes "Hardest Commutes" 2025; Swiss Re/Waymo Safety Study; Joby Aviation investor materials; Archer Aviation investor materials; U.S. Department of Transportation eIPP announcement, March 9, 2026; North Central Texas Council of Governments; Texas Comptroller's Office; Bureau of Economic Analysis.

---

*T&P Nexus Hub Transportation System | Fort Worth, Texas | March 2026*
*This white paper is provided for informational purposes. Financial projections are based on moderate scenario assumptions and are subject to change as project parameters are refined.*
